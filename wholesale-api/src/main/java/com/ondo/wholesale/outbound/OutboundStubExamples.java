...
/**
 * 계약 스텁 example — api-lite/06_출고 문서의 Response 예시 그대로. 실구현이 서비스
 * 호출로 교체하며 교체된 엔드포인트의 example 은 지운다. 파일 삭제는 출고 확정·장끼
 * 실구현(재고 원장 부품 합류 뒤)과 함께다.
 */
public final class OutboundStubExamples {
    ...
    public static List<OutboundRetailerResponse> outboundRetailers() {
        return List.of(new OutboundRetailerResponse(
                3307L, "RT-007", "부산 상사", 3, 66,
                OffsetDateTime.of(2026, 8, 12, 14, 20, 0, 0, KST), null));
    }

    public static List<OutboundSummaryResponse> outboundSummaries() {
    ...
    public static OutboundDetailResponse detailBeforeShip() {
        return detail(null, null, true);
    }

    /** 출고 확정 직후 상세 — shippedAt·statementNumber 채워짐. */
    public static OutboundDetailResponse detailAfterShip() {
    ...
    public static StatementResponse statement() {
