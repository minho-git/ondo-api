...
import com.ondo.wholesale.product.service.ListingSeasonService;
...
/**
 * 게시글 시즌 전이 (MUL-94) — 계약 원본: api-lite/02_상품게시.
...
public class ListingController {
    ...
    private final ListingSeasonService listingSeasonService;
    ...
    public ListingResponse seasonEnd(@AuthenticationPrincipal WholesalePrincipal principal,
                                     @PathVariable Long listingId) {
        return listingSeasonService.endSeason(principal.wholesalerId(), listingId);
    }
    ...
    @PostMapping("/reopen")
    public ListingResponse reopen(@AuthenticationPrincipal WholesalePrincipal principal,
                                  @PathVariable Long listingId) {
        return listingSeasonService.reopen(principal.wholesalerId(), listingId);
    }
}
