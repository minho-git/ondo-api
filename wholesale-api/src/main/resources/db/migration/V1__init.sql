...
CREATE UNIQUE INDEX spring_session_id_uk         ON wholesale.spring_session (session_id);
CREATE INDEX        spring_session_expiry_idx    ON wholesale.spring_session (expiry_time);
CREATE INDEX        spring_session_principal_idx ON wholesale.spring_session (principal_name);
...
-- GRANT USAGE ON ALL SEQUENCES IN SCHEMA wholesale TO wholesale_api;   -- bigserial 채번. 없으면 INSERT 가 전부 막힌다

-- 앞으로 만들 테이블에도 자동으로 붙는다. 위 GRANT 는 지금 있는 것만 잡는다
-- (테이블을 만드는 그 역할로 실행해야 한다)
-- ALTER DEFAULT PRIVILEGES IN SCHEMA common
--     GRANT SELECT ON TABLES TO wholesale_api;
