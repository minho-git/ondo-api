...
class OrderStatusesTest {
    ...
    void 취소된_주문은_출고와_무관하게_CANCELLED다() {
        OrderStatuses.Derived d = OrderStatuses.derive(OrderStatus.CANCELLED, 10, 4, 2);
    ...
    void 신규_주문은_확정과_취소만_할_수_있다() {
        OrderStatuses.Derived d = OrderStatuses.derive(OrderStatus.NEW, 10, 0, 0);
    ...
    void 확정_후_미출고면_CONFIRMED고_배분_잔량이_있으면_포장할_수_있다() {
        OrderStatuses.Derived d = OrderStatuses.derive(OrderStatus.CONFIRMED, 10, 4, 0);
    ...
    void 전량_배분된_확정_주문은_포장할_수_없다() {
        OrderStatuses.Derived d = OrderStatuses.derive(OrderStatus.CONFIRMED, 10, 10, 0);
        assertThat(d.key()).isEqualTo(OrderStatusKey.CONFIRMED);
        assertThat(d.packable()).isFalse();
    }
    ...
    void 일부_출고면_PARTIALLY_SHIPPED다() {
        OrderStatuses.Derived d = OrderStatuses.derive(OrderStatus.CONFIRMED, 10, 10, 3);
        assertThat(d.key()).isEqualTo(OrderStatusKey.PARTIALLY_SHIPPED);
    }

    @Test
    void 전량_출고면_SHIPPED고_아무_버튼도_안_뜬다() {
        OrderStatuses.Derived d = OrderStatuses.derive(OrderStatus.CONFIRMED, 10, 10, 10);
    ...
    @Test
    void 키마다_한글_라벨이_있다() {
        assertThat(OrderStatuses.label(OrderStatusKey.NEW)).isEqualTo("신규 주문");
        assertThat(OrderStatuses.label(OrderStatusKey.CONFIRMED)).isEqualTo("주문 확정");
        assertThat(OrderStatuses.label(OrderStatusKey.PARTIALLY_SHIPPED)).isEqualTo("부분 출고");
        assertThat(OrderStatuses.label(OrderStatusKey.SHIPPED)).isEqualTo("출고 완료");
        assertThat(OrderStatuses.label(OrderStatusKey.CANCELLED)).isEqualTo("주문 취소");
    }
}
