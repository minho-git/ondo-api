...
class OrderDispatchWorkerTest {
    ...
    void 밀린_건을_보낸다() {
        ...
        OrderGroup group = 대기_주문을_만든다("worker-1");

        worker.run();

        assertThat(상태(group)).isEqualTo(DispatchStatus.SENT);
        치운다(group);
    }
    ...
    void 실패하면_미룬다() {
        ...
        OrderGroup group = 대기_주문을_만든다("worker-2");

        worker.run();

        OrderDispatch row = 대기줄(group);
    ...
    void 사업상_거절은_끝낸다() {
        ...
        OrderGroup group = 대기_주문을_만든다("worker-3");

        worker.run();

        assertThat(상태(group)).isEqualTo(DispatchStatus.REJECTED);
        치운다(group);
    }
    ...
    void 기한을_넘기면_부르지_않는다() {
        OrderGroup group = 대기_주문을_만든다("worker-4");
        기한을_과거로_민다(group);
    ...
    void 집은_줄은_잠깐_안_보인다() {
        ...
        OrderGroup group = 대기_주문을_만든다("worker-5");
    ...
    private OrderDispatch 대기줄(OrderGroup group) {
        return dispatchRepository
                .findByOrderGroupIdAndWholesalerId(group.getId(), 무드온).orElseThrow();
    }

    private DispatchStatus 상태(OrderGroup group) {
        return 대기줄(group).getStatus();
    }

    /** 기한이 지난 상태를 만든다. 30분을 기다릴 수 없으니 DB 를 직접 민다. */
    private void 기한을_과거로_민다(OrderGroup group) {
        jdbc.update("update retail.order_dispatch set expires_at = ? where id = ?",
                OffsetDateTime.now().minusMinutes(1), 대기줄(group).getId());
    }

    /** 트랜잭션 없이 돌아서 커밋된다. 다음 테스트에 남으면 안 된다. */
    private void 치운다(OrderGroup group) {
