...
[무엇을 할 수 있나](#무엇을-할-수-있나) · [구성](#구성) · [API](#api) · [막혔던 곳](#막혔던-곳과-푼-방법) · [로컬에서 띄우기](#로컬에서-띄우기)
...
| | 무엇이 문제였나 | 결과 |
|---|---|---|
| [미수 잔액](#동시에-들어오면-미수-잔액이-틀어진다) | 동시 요청에 잔액이 덮어써지고, 취소하면 기록이 사라진다 | **89% 증발 → 0** |
| [대시보드](#주문이-쌓이면-대시보드가-느려진다) | 주문 300만 건에서 집계 6종이 354ms | **354ms → 0.2ms** |
| [접수 대기함](#도매가-멈추면-주문이-사라진다) | 도매가 멈추면 주문이 사라지고 사용자가 다시 보내야 한다 | **호출 85%↓ · 몰림 82%↓** |
| [상품 목록](#상품-목록이-느리다--재놓고-아직-안-넣었다) | 초당 20건에서 p95 5,059ms · 실패 100% | 28ms · 0% <sub>(미적용)</sub> |
